...
public class SpringGwtRemoteServiceServlet extends RemoteServiceServlet {
    ...
    private static final MyLogger PROFILE = new MyLogger();
    ...
    public String processCall(String payload) throws SerializationException {
        try {
            ...
            if (PROFILE.isDebugEnabled()) {
                PROFILE.debug(rpcRequest.getMethod().getDeclaringClass().getSimpleName() + "." + rpcRequest.getMethod().getName() + ",begin");
            }
            String resp = RPC.invokeAndEncodeResponse(handler, rpcRequest.getMethod(), rpcRequest.getParameters(), rpcRequest
                                                                                                                   .getSerializationPolicy());
            if (PROFILE.isDebugEnabled()) {
                PROFILE.debug(rpcRequest.getMethod().getDeclaringClass().getSimpleName() + "." + rpcRequest.getMethod().getName() + ",end," + (System.currentTimeMillis() - t0));
            }
            return resp;
